...
class ConfigMonitor:
    ...
    def update_coordinator(self):
        log.info("Update masters with new config")
        for pod in self.masters:
            self.provision_coordinator(pod)
    ...
    def provision_coordinator(self, pod_name: str) -> None:
        coordinator_provision = self.load_config_map(self.coordinator_provision_path)
        self.provision_node(coordinator_provision, pod_name, self.coordinator_service)
    ...
    def start_watchers(self):
        ...
        FileWatcher(self.update_coordinator, self.coordinator_provision_path).start()
